...
function scoreItem(item: ClothingItem, q: string): number {
  let score = 0;
  const hit = (val: string | null | undefined, pts: number) => {
    if (val && val.toLowerCase().includes(q)) score += pts;
  };
  hit(item.name,          10);
  hit(item.brand,          8);
  hit(item.color,          5);
  hit(SLOT_LABELS[item.category as SlotKey] ?? item.category, 4);
  hit(item.notes,          4);
  hit(item.size,           3);
  hit(item.season,         3);
  hit(item.occasion,       3);
  hit(item.purchasePrice,  2);
  hit(item.purchaseDate,   2);
  hit(item.createdAt,      1);
  if (item.visionLabels) for (const l of item.visionLabels) hit(l, 1);
  if (item.visionText)   for (const t of item.visionText)   hit(t, 1);
  return score;
}
...
function getSearchResults(
  ...
  const matchedItems = allItems
    .filter((item) => scoreItem(item, q) > 0)
  ...
  const matchedGroups = outfits.filter((outfit) => {
    ...
    return (outfit.items ?? []).some((item) => scoreItem(item, q) > 0);
  });
...
export default function SavedPage() {
  ...
  const [focusOutfitId, setFocusOutfitId] = useState<number | null>(null);
  const containerRef   = useRef<HTMLDivElement>(null);
  const renameInputRef = useRef<HTMLInputElement>(null);
  const notesInputRef  = useRef<HTMLTextAreaElement>(null);
  const outfitCardRefs = useRef<Record<number, HTMLDivElement | null>>({});
  ...
  // After clearing search to focus a group, scroll to its card
  useEffect(() => {
    if (!query && focusOutfitId != null) {
      const el = outfitCardRefs.current[focusOutfitId];
      if (el) {
        // Small timeout to let the list re-render
        setTimeout(() => el.scrollIntoView({ behavior: "smooth", block: "start" }), 50);
      }
      setFocusOutfitId(null);
    }
  }, [query, focusOutfitId]);
  ...
  const openItem = useCallback((item: ClothingItem, fromSearch = false) => {
    setDetailsItem(item);
  ...
  const searchResults = query.trim()
    ? getSearchResults(query, allItems, outfits ?? [])
    : null;

  return (
    <div
      ref={containerRef}
      className="min-h-full flex flex-col pt-8 px-4 pb-8 bg-secondary/10 relative overflow-y-auto"
    >
      {/* ── Header ── */}
      ...
      <div className="relative mb-5">
        ...
        <input
          ...
          className="w-full pl-9 pr-9 py-2.5 border-2 border-black rounded-xl text-sm font-medium
        ...
        {query && (
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full
                       bg-black/10 flex items-center justify-center hover:bg-black/20 transition-colors"
          >
      ...
      {searchResults ? (
        <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                {searchResults.items.map((item) => (
                  <button
                    ...
                    className="flex items-center gap-3 w-full p-3 bg-white border-2 border-black
                               rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-left
                               active:scale-[0.98] transition-all"
                  >
                    <div className="w-12 h-12 border-2 border-black rounded overflow-hidden bg-[#F5EDD8] shrink-0">
                      {item.imageObjectPath ? (
                        <img
                          src={getImageUrl(item.imageObjectPath)!}
                          alt={item.name}
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <span className="text-[9px] font-bold text-black/25">—</span>
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm truncate">{item.name || "Unnamed"}</p>
          ...
          {searchResults.groups.length > 0 && (
            <section>
              <p className="text-[10px] font-bold uppercase tracking-widest text-black/40 mb-2">
                Lookbooks — {searchResults.groups.length}
              </p>
              <div className="flex flex-col gap-2">
                    ...
                    >
                      ...
                      <div className="flex gap-1 shrink-0">
                        {thumbs.map((item) => (
                          <div
                            key={item.id}
                            className="w-10 h-10 border-2 border-black rounded overflow-hidden bg-[#F5EDD8]"
                          >
                            {item.imageObjectPath ? (
                              <img
                                src={getImageUrl(item.imageObjectPath)!}
                        ...
                        {Array.from({ length: Math.max(0, 3 - thumbs.length) }).map((_, i) => (
                          <div
                            key={`ph-${i}`}
                            className="w-10 h-10 border-2 border-dashed border-black/20 rounded bg-black/5"
                          />
                        ))}
                      </div>
                      <div className="flex-1 min-w-0">
          ...
          {searchResults.items.length === 0 && searchResults.groups.length === 0 && (
            <div className="flex flex-col items-center justify-center text-center py-16 gap-3">
              <Search className="w-10 h-10 text-black/15" />
              <p className="text-sm font-medium text-black/40">
                Nothing matched "<span className="font-bold">{query}</span>"
              </p>
            </div>
          )}
        </div>
            ...
            <div className="flex flex-col gap-6">
              {outfits.map((outfit) => {
                ...
                return (
                  <motion.div
                    key={outfit.id}
                    ref={(el) => { outfitCardRefs.current[outfit.id] = el; }}
                        ...
                        {SLOT_ORDER.map((slot) => {
                          ...
                          return (
                            <div key={slot} className="flex flex-col gap-0.5">
                              {item ? (
                                <>
                                  <ItemPhoto item={item} size="lg" onClick={() => openItem(item)} />
